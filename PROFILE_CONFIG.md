...
| `extIf`                | The external interface which connects to the Internet or to the network you want to reach through the VPN                                                                   | yes      | _N/A_                      | from `deploy_${DIST}.sh`   |
...
| `enableNat4`           | Whether or not to NAT the `range` network to `extIf`                                                                                                                        | no       | `false`                    | `true`                     |
| `enableNat6`           | Whether or not to NAT the `range6` network to `extIf`                                                                                                                       | no       | `false`                    | `true`                     |
| `reject4`              | Do not forward IPv4 traffic, useful for creating an IPv6 only VPN                                                                                                           | no       | `false`                    | `false`                    |
| `reject6`              | Do not forward IPv6 traffic, useful when the VPN server does not have IPv6 connectivity                                                                                     | no       | `false`                    | `false`                    |
