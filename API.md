...
### TOTP
    ...
    $ curl -H "Authorization: Bearer abcdefgh" \
        -d "totp_secret=K4JMMGO7T4ZR7ROZ&totp_key=123456" \
        https://demo.eduvpn.nl/portal/api.php/two_factor_enroll_totp

The `totp_secret` is a Base32 encoded (only upper case) string made up of 10 
random bytes (80 bits). The `totp_key` contains a TOTP key as generated by the
TOTP application.
